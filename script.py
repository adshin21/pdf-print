...
import os
path = "/Users/adityak.umar/xl/test_files"
...
for file in files:
	...
	print("\n\nFILE is => ", file)
	data, number_of_rows = get_raw_data(path + '/'  + file)
	before_table_data = data[:5]
	table_data = data[5: -2]
	after_table_data = data[-2: ]

	print("What is before table data", after_table_data)

	from create_pdf import create_pdf
	create_pdf(file_name=file.split('.')[0],  before_table=before_table_data[:], table=table_data[:], after_table=after_table_data[:])
	for i in data:
		print(*i, sep=', ')
